...
gpargs = gparguments(sol1, t1; colortype = :relrating, ecolorscaling = Observable(1.0), offlinecolor=colorant"lightgray")
...
gpargs_final = gparguments(sol1, sol1.sol.t[end]; colortype = :relrating, ecolorscaling = Observable(1.0), offlinecolor=colorant"lightgray")
...
leftpane[3, 1] = Colorbar(fig, height = 25, vertical = false,
    colormap = DynamicCascades.edge_colorsheme(edgecolortype), label = "STATIC failures: initial load (top), final load (bottom) relative to rating")
...
ylims!(ax2, 0, 1.1)
xlims!(ax2, 0, 50)
save(joinpath(PLOT_DIR, "isolator_toymodel_static_fails.pdf"), fig)
