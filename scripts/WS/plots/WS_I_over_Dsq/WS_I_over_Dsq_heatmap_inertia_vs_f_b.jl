...
if length(filtered_freq_bounds) > 1
    ...
    lines!(ax_hm, [NaN], [NaN]; label=L"Damping $D=\sqrt{I}$ [$s$]", color=:white)
...
CairoMakie.save(joinpath(MA_DIR, "WS", "WS_I_over_Dsq_heatmap_log=$heatmap_logscale,sumlinesnodes=$sum_lines_nodes,K=$K_str,k=$k_str,β=$filtered_β_values,f_b=$filtered_freq_bounds_str.pdf"),fig_hm)
CairoMakie.save(joinpath(MA_DIR, "WS", "WS_I_over_Dsq_heatmap_log=$heatmap_logscale,sumlinesnodes=$sum_lines_nodes,K=$K_str,k=$k_str,β=$filtered_β_values,f_b=$filtered_freq_bounds_str.png"),fig_hm)
fig_hm
