...
exp_name_date = "WS_k=4_exp06_1_I_over_D_lines_PIK_HPC_K_=3,N_G=32_20250124_131053.306"
...
lines!(ax_lines_only, [NaN], [NaN]; label=L"Damping $D=I$ [$s$]", color=:white)
axislegend(ax_lines_only, position = :rt, labelsize=labelsize)

k_str = string(exp_params_dict[:k])
filtered_freq_bounds_str = string(filtered_freq_bounds)
K_str = string(exp_params_dict[:K])

CairoMakie.save(joinpath(MA_DIR, "WS", "WS_I_over_D_lines_only_K=$K_str,k=$k_str,β=$filtered_β_values,M_left_out=$left_out_inertia_values.png"),fig_lines_only)
CairoMakie.save(joinpath(MA_DIR, "WS", "WS_I_over_D_lines_only_K=$K_str,k=$k_str,β=$filtered_β_values,M_left_out=$left_out_inertia_values.pdf"),fig_lines_only)
fig_lines_only
