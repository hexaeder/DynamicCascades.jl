...
print("solve(prob, Rodas4P(); callback=cbs, progress=true, solverargs...\n")
print("solverargs = (;dtmax=0.01), verbose = true);")
...
freq_bound = round(2.0/(2*π), digits=2)
# create folder
t=now()
datetime = Dates.format(t, "yyyymmdd_HHMMSS.s") # https://riptutorial.com/julia-lang/example/20476/current-time
folder = string("/",datetime,"more_cpus_inertia_vs_node_failures_f_bound=$freq_bound")
