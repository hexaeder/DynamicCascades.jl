...
# line_colors = ["#000000FF", "#4D4D4DFF", "#8C8C8CFF", "#CFCFCFFF"]
...
ax_lines_only = Axis(fig_lines_only[1, 1],
    ...
    # ylabel = normalize ? "normalized average of line failures" : L"# Line failures $\left< l \right>$",
    ...
    if M == maximum(filtered_inertia_values)
        ...
        if (trip_lines == :dynamic &&  trip_nodes == :none)
            if freq_bound == filtered_freq_bounds[1]
                scatterlines!(ax_lines_only, filtered_inertia_values, y_lines, marker = marker, markersize = markersize, linewidth=linewidth, label = "β=$β", color = line_colors[color_index])
...
# Label(fig_lines_only[1, 1, TopLeft()], "c", fontsize = fontsize+labellettersize, font = :bold, padding = (-95, 0, 50, 0))

xlims!(ax_lines_only, 0, 30.5)
k_str = string(exp_params_dict[:k])
filtered_freq_bounds_str = string(filtered_freq_bounds)
K_str = string(exp_params_dict[:K])

CairoMakie.save(joinpath(MA_DIR, "WS_vary_I_only_lines_only_K=$K_str,k=$k_str,β=$filtered_β_values,M_left_out=$left_out_inertia_values.pdf"),fig_lines_only)
# CairoMakie.save(joinpath(MA_DIR, "WS_vary_I_only_lines_only_K=$K_str,k=$k_str,β=$filtered_β_values,M_left_out=$left_out_inertia_values.png"),fig_lines_only)
fig_lines_only