...
μ = round(mean(braessness_nodes), digits=3) # 0.344
# hist!(ax11, braessness_nodes; label=L"<$B^n$>=0.34", color=:red, bins=bins, strokewidth=0.2)
hist!(ax11, braessness_nodes; label=L"Node failures, $\left< B \right>=0.34$", color=:red, bins=bins, strokewidth=0.2)
...
μ = round(mean(braessness_lines), digits=3) # 4.446
# hist!(ax21, braessness_lines; label=L"<$B^l$>=4.45", color=:orange, bins=bins, strokewidth=0.2)
hist!(ax21, braessness_lines; label=L"Line failures, $\left< B \right>=4.45$", color=:orange, bins=bins, strokewidth=0.2)
...
hist!(ax31, braessness_lines .+ braessness_nodes; label=L"Node and line failures, $\left< B \right> = 4.79$", color="#8C8C8CFF", bins=bins, strokewidth=0.2)
axislegend(ax31; position = :rt)
ax11.yticks = ax21.yticks = ax31.yticks = [0, 1, 10, 100, 1000, 5000]
ax11.xticks = ax21.xticks = ax31.xticks = [-10, 0, 10, 20, 30, 40, 50, 60]
...
Label(fig[1, 1, TopLeft()], fontsize=fontsize+labellettersize, "a", font = :bold, padding = (-55, 0, 5, 0))
Label(fig[2, 1, TopLeft()], fontsize=fontsize+labellettersize, "c", font = :bold, padding = (-55, 0, 5, 0))
Label(fig[3, 1, TopLeft()], fontsize=fontsize+labellettersize, "e", font = :bold, padding = (-55, 0, 5, 0))

CairoMakie.save(joinpath(MA_DIR, "histograms_braessness_WS_lines,nodes_separately_$model,N=$N_nodes,f_b_n=$f_b_narrow,f_b_w=$f_b_wide,I=$I,exclude_non_triggering=$exclude_non_triggering.pdf"),fig)
# CairoMakie.save(joinpath(MA_DIR, "histograms_braessness_WS_lines,nodes_separately_$model,N=$N_nodes,f_b_n=$f_b_narrow,f_b_w=$f_b_wide,I=$I,exclude_non_triggering=$exclude_non_triggering.png"),fig)
fig
