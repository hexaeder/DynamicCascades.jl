...
if length(filtered_freq_bounds) > 1
    ...
    #     if i == 1
    #         scatter!(ax_hm, filtered_freq_bounds[i], opt_inertia[i], color = Makie.wongsize_colors()[1], label = L"$I_{min}$: Inertia value associated with minimum of failures", markersize = markersize)
    #     end
    ...
    ax_hm.xticks = [0.005, 0.025, 0.045, 0.065, 0.085, 0.110, 0.130, 0.150]
    ax_hm.xlabelpadding = 15
    Label(fig_hm[1, 1, TopLeft()], fontsize=fontsize+labellettersize, "a", font = :bold, padding = (-55, 0, 5, 0))

end
CairoMakie.save(joinpath(MA_DIR, "WS_vary_I_only_heatmap_log=$heatmap_logscale,sumlinesnodes=$sum_lines_nodes,K=$K_str,k=$k_str,β=$filtered_β_values,equidistant=$equidistant_inertia_values.pdf"),fig_hm)
# CairoMakie.save(joinpath(MA_DIR, "WS_vary_I_only_heatmap_log=$heatmap_logscale,sumlinesnodes=$sum_lines_nodes,K=$K_str,k=$k_str,β=$filtered_β_values,equidistant=$equidistant_inertia_values.png"),fig_hm)
fig_hm
