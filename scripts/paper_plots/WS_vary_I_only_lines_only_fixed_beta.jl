...
ax_lines_only = Axis(fig_lines_only[1, 1],
    title = "Line-failure-only model (WS)",
    titlefont = :regular,
    ...
    if M == maximum(filtered_inertia_values)
        ...
        if (trip_lines == :dynamic &&  trip_nodes == :none)
            if freq_bound == filtered_freq_bounds[1]
                scatterlines!(ax_lines_only, filtered_inertia_values, y_lines, linewidth=linewidth, marker = marker, markersize = markersize, label = "k=$k,β=$β", color = line_colors[color_index])
