...
exp_name_date = "WS_k=4_exp08_vary_alpha_lines_and_nodes_PIK_HPC_K_=3,N_G=32_20250326_231041.385"
...
if length(filtered_α_values) > 1
    ...
    Label(fig_hm[1, 1, TopLeft()], fontsize=fontsize+labellettersize, labelstring, font = :bold, padding = (-55, 0, 5, 0))

    ax_hm.xticks = filtered_α_values
    ax_hm.xlabelpadding = 15

end
CairoMakie.save(joinpath(MA_DIR, "WS_vary_α_heatmap_log=$heatmap_logscale,sumlinesnodes=$sum_lines_nodes,K=$K_str,k=$k_str,β=$filtered_β_values,f_b=$filtered_freq_bounds_str,D=$D,equidistant=$equidistant_inertia_values.pdf"),fig_hm)
# CairoMakie.save(joinpath(MA_DIR, "WS_vary_α_heatmap_log=$heatmap_logscale,sumlinesnodes=$sum_lines_nodes,K=$K_str,k=$k_str,β=$filtered_β_values,f_b=$filtered_freq_bounds_str,D=$D,equidistant=$equidistant_inertia_values.png"),fig_hm)
fig_hm