...
function simulate_wrapper(exp_name_date, task_id, initial_fail;
    ...
    N,k,β,graph_seed,μ,σ,distr_seed,K,α,M,γ,τ,freq_bound,_,_,init_pert,ensemble_element = get_network_args_stripped(df_config, task_id)
...
####
df_config = DataFrame(CSV.File(joinpath(RESULTS_DIR, exp_name_date_full, "config.csv")));
# df_config = DataFrame(CSV.File(joinpath(RESULTS_DIR, exp_name_date_inertia, "config.csv")));
df_config[task_id_inertia_n, :]

####

tweak_power_injections = false
# Network from ensemble
sol1 = simulate_wrapper(exp_name_date_full, 2265, 95; save_simulation = true, tweak_power_injections=tweak_power_injections, trip_lines=:none, trip_nodes=:none); # no failures
sol4 = simulate_wrapper(exp_name_date_full, 2265, 95; save_simulation = true, tweak_power_injections=tweak_power_injections, trip_lines=:dynamic, trip_nodes=:dynamic); # narrow inertia failure
...
inset_lims = [-.0, 2.35, -1.5, 0.] # [2.495, 3.8, -1.2, 1.2] # x0, x1, y0, y1  # [-3.0, 3.8, -3.1, 1.2] 
selected_lines = [88, 97] # selected_lines = sol4.failures.saveval[1:21]

# selected_lines = collect(1:200)
...
times = [0.09000000000000001, 1.23, 4, 5, 7.5]
xticks_ax_gc21 = round.([0.1, 1.23, 4, 5, 7.5], digits=1)
...
gb[1,2] = plotnetwork(fig, sol4, times[2]; inset=inset_lims, show_labels=false)
...
gb[2,1] = Label(fig, "(c no fail) t = $(round(times[3],digits=2)) s", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)

gb[2,2] = plotnetwork(fig, sol1, times[4]; inset=inset_lims, show_labels=false)
gb[2,2] = Label(fig, "(d no fail) t = $(round(times[4],digits=2)) s", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)

gb[2,3] = plotnetwork(fig, sol1, times[5]; inset=inset_lims, show_labels=false)
gb[2,3] = Label(fig, "(e no fail) t = $(round(times[5],digits=2)) s", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)

gb[2,4] = plotnetwork(fig, sol1, sol1.sol.t[end] - h; inset=inset_lims, show_labels=false)
gb[2,4] = Label(fig, "(f no fail) t → ∞ \n (steady state)", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)
...
gb[3,1] = Label(fig, "(cIn) t = $(round(times[3],digits=2)) s", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)

gb[3,2] = plotnetwork(fig, sol4, times[4]; inset=inset_lims, show_labels=false)
gb[3,2] = Label(fig, "(dIn) t = $(round(times[4],digits=2)) s", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)

gb[3,3] = plotnetwork(fig, sol4, times[5]; inset=inset_lims, show_labels=false)
gb[3,3] = Label(fig, "(eIw) t = $(round(times[5],digits=2)) s", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)

gb[3,4] = plotnetwork(fig, sol4, sol4.sol.t[end] - h; inset=inset_lims, show_labels=false)
gb[3,4] = Label(fig, "(fIw) t → ∞ \n (steady state)", tellwidth=false, tellheight=false, halign=:right, valign=:bottom)
...
## trajectories
gc[1,1] = ax_gc11 = Axis(fig; xlabel="time t in s", ylabel="apparent power flow in p.u.", title="no failures (solid), inertia failure model narrow bounds (dashed)", titlealign=:left)
